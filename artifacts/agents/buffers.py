...
class HRLReplayBuffer:
    ...
    def __init__(
        ...
        self.option_probs = np.zeros((capacity, option_dim), dtype=np.float32)
    ...
    def push_strategic(
        ...
        self.option_probs[self.ptr] = option_probs
        self.next_obs[self.ptr] = next_obs
    ...
    def sample_strategic(self, batch_size: int, device: str = "cpu") -> Tuple:
        ...
        return (
            ...
            torch.FloatTensor(self.next_obs[idx]).to(device),
            torch.FloatTensor(self.dones[idx]).to(device),
            torch.FloatTensor(self.option_probs[idx]).to(device),  # stored π_old for IS
        )
    ...
    def sample_tactical(self, batch_size: int, device: str = "cpu") -> Tuple:
        ...
        idx = np.random.randint(0, self.size, size=min(batch_size, self.size))
        option_dim = self.option_probs.shape[1]
...
class RolloutBuffer:
    ...
    def flush_strategic(self, gamma: float, buffer):
        """Store ONE transition per option: (s_start, ω, Σγᵏr_k, s_end) — not K duplicates."""
        if not self.option_ptr_indices:
            self.option_step_rewards = []
            self.option_ptr_indices = []
            return
        # Total discounted return from option start to end
        G = 0.0
        for r in reversed(self.option_step_rewards):
            G = r + gamma * G
        # Only store start state → end state transition
        start_idx = self.option_ptr_indices[0]
        end_idx = self.option_ptr_indices[-1]
        next_idx = min(end_idx + 1, self.size - 1)
        buffer.push_strategic(
            self.obs[start_idx].copy(),
            self.options[start_idx],
            self.option_probs[start_idx].copy(),
            self.obs[next_idx].copy(),
            G,
            self.dones[start_idx, 0] > 0.5,
