...
class PPOPolicy(nn.Module):
    ...
    def __init__(self, state_dim: int, action_dim: int, hidden: int = 256):
        ...
        # Learned log_std: init std≈0.6 for 136-dim space (not std=1.0 which causes chaos)
    ...
    def get_action_and_value(self, x, action=None):
        ...
        # Squashed Gaussian log-prob: log π(a|s) = log N(z|μ,σ) - Σ log(1 - tanh²(z))
        log_prob = probs.log_prob(z).sum(-1)
    ...
    def select_action(
        ...
        with torch.no_grad():
            ...
            if deterministic:
                ...
                # Compute log_prob for the deterministic action (same squashed path)
                logstd = self.policy.actor_logstd.expand_as(action_mean)
                probs = Normal(action_mean, torch.exp(logstd))
                log_prob = probs.log_prob(action_mean).sum(-1)
                log_prob -= torch.log(1 - action.pow(2) + 1e-6).sum(-1)
