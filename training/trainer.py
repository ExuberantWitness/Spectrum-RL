...
class HRLTrainer:
    ...
    def train(self) -> Dict:
        ...
        option_start_step = 0
        option_logits = np.zeros(self.cfg.hrl.strategic_options, dtype=np.float32)
        ...
        while self.total_steps < self.cfg.total_steps:
            ...
            should_replan = (
                ...
                or episode_step - option_start_step >= self.cfg.hrl.option_duration_max
            )

            if should_replan and self.strategic_buffer.size > 100:
                obs_tensor = torch.FloatTensor(obs).unsqueeze(0).to(self.device)
                term_prob = self.strategic.get_termination(
                    obs_tensor,
                    torch.FloatTensor(option_onehot).unsqueeze(0).to(self.device),
                ).item()
                should_replan = should_replan or (np.random.random() < term_prob)
            ...
            if should_replan or current_option is None:
                ...
                with torch.no_grad():
                    ...
                    option_logits = torch.softmax(logits, dim=-1).squeeze(0).cpu().numpy()
                    option_onehot = np.zeros(self.cfg.hrl.strategic_options, dtype=np.float32)
                    option_onehot[current_option] = 1.0
                option_start_step = episode_step

            # --- Tactical layer (PPO): allocate resources ---
            obs_tensor = torch.FloatTensor(obs).unsqueeze(0).to(self.device)
            ...
            scaled_reward = reward * self.cfg.hrl.reward_scale
            ...
            self.rollout.push(
                obs.copy(), current_option, option_logits.copy(), option_onehot.copy(),
    ...
    def _update_strategic(self, batch: Tuple):
        obs, options, returns, next_obs, dones = batch

        option_logits, values, _ = self.strategic(obs)
        option_log_probs = torch.log_softmax(option_logits, dim=-1)
        selected_log_probs = option_log_probs.gather(1, options.unsqueeze(-1))
        ...
        value_loss = nn.MSELoss()(values, returns_norm)
