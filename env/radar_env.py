...
class IntegratedRadarEnv(gym.Env):
    ...
    def _setup_spaces(self):
        ...
        #               target_params(4*targets), function_status(4),
    ...
    def _setup_models(self):
        ...
        self.time_step = 0
        self._function_history = np.zeros((4, 100), dtype=np.float32)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        # Update history
        func_idx = min(self.time_step, 99)
        self._function_history[0, func_idx] = detect_result.get("success", 0.0)
        self._function_history[1, func_idx] = recon_result.get("coverage", 0.0)
        self._function_history[2, func_idx] = jam_result.get("effectiveness", 0.0)
        self._function_history[3, func_idx] = comm_result.get("rate", 0.0)
    ...
    def _parse_action(self, action: np.ndarray) -> dict:
        ...
        # Power levels for 4 functions
        power_raw = action[idx:idx + 4]
        power_alloc = self._softmax(power_raw) * 10 ** (self.cfg.max_power_db / 10.0)
    ...
    def _simulate_detection(self, resources: dict) -> dict:
        ...
        for tgt in self.targets:
            # Find best beam pointing roughly at target
            beam_gain_loss = 0.0
            for i in range(c.num_beams):
                ang_err = abs(resources["beam_az"][i] - tgt.angle_deg)
                if ang_err < 5.0:
                    beam_gain_loss = (ang_err / 5.0) ** 2 * 3.0  # 3 dB at beam edge
                    break
            else:
    ...
    def _simulate_reconnaissance(self, resources: dict) -> dict:
        ...
        detected_signal = (channel_gains + freq_sensing) > -100.0
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # Target parameters
        target_params = np.zeros(4 * c.num_targets, dtype=np.float32)
        for i, tgt in enumerate(self.targets):
        ...
        # Function status
        func_status = np.array([
            self._function_history[0, min(self.time_step, 99) or 0],
            self._function_history[1, min(self.time_step, 99) or 0],
            self._function_history[2, min(self.time_step, 99) or 0],
            self._function_history[3, min(self.time_step, 99) or 0],
        ], dtype=np.float32)
